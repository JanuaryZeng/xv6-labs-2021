...
e1000_transmit(struct mbuf *m)
{
  ...
  //
    ...
    uint32 tail = regs[E1000_TDT];// 下一个可用的 buffer 的下标
    struct tx_desc *desc = &tx_ring[tail];// 获取 buffer 的描述符，其中存储了关于该 buffer 的各种信息
...
e1000_recv(void)
{
  ...
  //
    while(1) { // 每次 recv 可能接收多个包
        ...
        net_rx(rx_mbufs[tail]); // 传递给上层网络栈。上层负责释放 mbuf
